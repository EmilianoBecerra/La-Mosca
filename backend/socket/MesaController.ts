...
export class MesaController {
  ...
  async crearNuevaMesa(socket: Socket, nombreJugador: string, nombreMesa: string, password: string) {
    try {
  ...
  async sumarJugador(socket: Socket, nombreJugador: string, nombreMesa: string, password: string) {
    try {
  ...
  async iniciarPartida(socket: Socket, mesa: Mesa) {
    try {
  ...
  async recibirDescarte(socket: Socket, nombreJugador: string, indices: number[], nombreMesa: string) {
    try {
      ...
      if (resultado.ok) {
        ...
        if (mesa) {
          const jugadores: Jugador[] = mesa.jugadores;
          if (jugadores) {
  ...
  async procesarDescarte(socket: Socket, mesa: Mesa, jugadores: Jugador[]) {
    try {
  ...
  iniciarNuevaMano(socket: Socket, nombreMesa: string) {
    try {
  ...
  async borrarMesa(socket: Socket, nombreMesa: string) {
    try {
  ...
  private async sincronizarYEmitirMesas(socket: Socket) {
    try {
